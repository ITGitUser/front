...
const Board = () => {
  ...
  const [boardData, setBoardData] = useState(
    DataPro.find((project) => project.idProject === idParam.id)
  );
  const [activeMenu, setActiveMenu] = useState(1);
  const [modalActive, setModalActive] = useState(false);
  const [nameColumn, setNameColumn] = useState("");
  const [color, setColor] = useState("#aabbcc");
  const [activeViewButton, setActiveViewButton] = useState(
    parseInt(localStorage.getItem("activeViewButtonIndex_Board"))
  );
  const [progress, setProgress] = useState(0);
  const [flexDirection, setFlexDirection] = useState(
    localStorage.getItem("flexDirectionForWrapperCards_Board")
  );
  ...
  const changeStyle = (
    ...
    //  r1-dev, r0-build
    const elementDropableAreaEnd = document.querySelector(
    ...
    if (idDropAreaEnd !== idDropAreaStart) {
      ...
      for (let i = elementDivIntoDragEnd.length - 1; i >= index; i--) {
        countHeight =
          countHeight + elementDivIntoDragEnd[i].clientHeight + 11.5;
      }
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "activeViewButtonIndex_Board",
      activeViewButton.toString()
    );
      ...
      if (type === "menu") {
        ...
        buttons.push(
          <button
            ...
            onClick={() => {
              setActiveViewButton(i);
            }}
          >
            <Icons
              name={array[i]}
              color={activeViewButton === i ? "#365EFD" : "black"}
              size="20px"
            />
            {array[i]}
          </button>
  ...
  const drawGroupCards = () => {
    ...
    return (
      <div style={flexContainerStyle} className={styles.wrapperCards}>
  ...
  const onDragUpdate = (update) => {
    ...
    if (destination) {
      if (type === "CARD") {
        ...
        const currentNum =
          boardData.columns.find((col) => col.id === draggedDroppableId).taskIds
            .length - draggedIndex;
        changeStyle(
          currentNum,
          draggedDroppableId,
          draggedDroppableIdStart,
          draggedIndex,
          source.index
        );
      ...
      <div className={styles.wrapperBoard}>
          ...
          <div className={styles.HeaderBoard}>
            ...
            <div className={styles.menu2}>
              <div className={styles.wrapperButtonAddAndUsers}>
                ...
                <div className={styles.ButtonAddMembers}>
                  <ButtonAdd text="+ Добавить участника" />
                </div>
              </div>
              <button
        ...
        {boardData && (
          <DragDropContext onDragUpdate={onDragUpdate} onDragEnd={onDragEnd}>
            <Droppable
              droppableId={boardData.idProject}
              direction={activeViewButton !== 2 ? "horizontal" : "vertical"}
              type="COLUMN"
            >
              {(provided, snapshot) => (
                <div
                  ...
                  style={{
                    display: "flex",
                    flexDirection: activeViewButton !== 2 ? "row" : "column",
                    width: "100%",
      ...
      <ModalWindow active={modalActive} setActive={setModalActive}>
        ...
        <form action="" onSubmit={handleSubmit}>
          ...
          <button
            ...
            onClick={() =>
              nameColumn !== ""
                ? (AddColumnOnBoard(nameColumn), setModalActive(false))
                : ""
            }
          >
            Добавить
